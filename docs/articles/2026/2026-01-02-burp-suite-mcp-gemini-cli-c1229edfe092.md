...
description: "Connect Burp Suite to Gemini CLI using Model Context Protocol (MCP) and Turn Burp into an AI-callable toolset and accelerate recon\u2026"
image: "https://cdn-images-1.medium.com/max/800/1*BDe41fzpmr9R53fn5CbjcQ.png"
---
...
- **Category:** CTI
- **Source article:** [https://medium.com/@1200km/burp-suite-mcp-gemini-cli-c1229edfe092](https://medium.com/@1200km/burp-suite-mcp-gemini-cli-c1229edfe092)
...
- Go to**Extensions**(or**Extender**) →**BApp Store**
...
- Start**Burp Suite**and ensure the MCP extension is**Enabled**

- Start**Gemini CLI**

- In Gemini CLI, run the MCP listing command (interactive command support includes`/mcp`).[Google Cloud Documentation](https://docs.cloud.google.com/gemini/docs/codeassist/gemini-cli)
...
- **MCP initiated**and Burp MCP used; proxy history retrieval initially failed due to missing offset, then proceeded

- **Traffic sent to Intruder**for parameter manipulation/testing

- **nmap service discovery**identified broad attack surface including Java serialization, Tomcat, Jetty, Samba

- **Port 443 follow-up**remained unidentified by nmap version detection

- **SQLi confirmed**via sqlmap on`pic_id`(time-based blind); attempts to enumerate/extract were slow/unstable

- **RCE condition demonstrated**through upload behavior (.htaccess in images + server-side execution claim in report draft; upload result visible in UI)

- **Exploitation attempts**did not result in a stable shell; reporting was initiated
...
- **Critical — Remote Code Execution via Upload Handling**
...
- **High — Time-Based Blind SQL Injection (**`**pic_id**`**)**
...
- **Medium–High — Broad Exposed Attack Surface (Services/Middleware)**
