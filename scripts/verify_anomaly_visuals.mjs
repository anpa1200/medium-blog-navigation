...
const dir=resolve(root,'reports/anomaly-visuals-20260921');mkdirSync(dir,{recursive:true});
const steps=[
  ...
  ['visual-browser','node',['scripts/check_anomaly_visuals_browser.mjs']],
...
for(const [id,command,args] of steps){
  ...
  if(id.endsWith('-build')){
    ...
    const files=['manifest.json','research-map.png','cover-provenance.json',...manifest.figures.flatMap(f=>Object.values(f.assets).map(a=>a.name))];
